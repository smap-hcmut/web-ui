...
export function ScopeFilter() {
  ...
  // Get active campaign's projects
  const activeCampaign = useMemo(
    () => campaigns.find((c) => c.id === activeCampaignId) ?? null,
    [activeCampaignId],
  );
  const projects = activeCampaign?.projects ?? [];
  ...
  // Collect selected labels for chips (only project + keyword)
  ...
  return (
    <div ref={panelRef} className="relative">
      ...
      {/* Dropdown panel — tree picker (project > keyword) */}
      {open && (
        <div
        ...
        >
          {projects.length === 0 && (
            <p className="text-[12px] px-4 py-3" style={{ color: 'var(--text-muted)' }}>
              {activeCampaignId ? 'No projects in this campaign' : 'No campaign selected'}
            </p>
          )}
                ...
                {projExpanded && proj.keywords.map((kw) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center gap-2 pl-10 pr-3 py-1.5 w-full transition-colors"
